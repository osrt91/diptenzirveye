...
/* ══════════════════════════════════════════════════════
   ...
   v2 — Mevcut proje desteği, seviye bazlı öneriler,
         anlık KULLAN/KULLANMA paneli
══════════════════════════════════════════════════════ */
...
function generateRecs(a) {
  ...
  const use = [], avoid = [], warnings = [], fixes = [], tips = [];
  const lvl = userLevel || 0; // 0=acemi, 1=orta, 2=ileri
  ...
  // Tekrar eden tool'ları temizle
  const seen = new Set();
  const dedup = arr => arr.filter(x => { if (seen.has(x.tool)) return false; seen.add(x.tool); return true; });
...
export default function App() {
  ...
  const toggleArr = (k, v) => { const s = a[k]; set(k, s.includes(v) ? s.filter(x => x !== v) : [...s, v]); };
  ...
  const C = (active, opts) => ({
    ...
    boxShadow: active ? "0 2px 12px rgba(249,115,22,.3)" : "none", textAlign: "left",
  });
  const inp = { width: "100%", padding: "11px 14px", borderRadius: 8, border: "1.5px solid #e8e2da", fontSize: 14, fontFamily: "'DM Sans'", outline: "none", marginBottom: 16, boxSizing: "border-box", background: "#fdfaf7", color: "#0d0d0d" };
  const H2 = ({ t, d }) => (<><h2 style={{ fontSize: 26, fontWeight: 800, color: "#0d0d0d", fontFamily: "'Syne'", letterSpacing: "-.02em", marginBottom: 4 }}>{t}</h2><p style={{ fontSize: 14, color: "#9a9590", marginBottom: 22, fontFamily: "'DM Sans'", lineHeight: 1.7 }}>{d}</p></>);
  const Lbl = ({ t }) => <div style={{ fontWeight: 700, fontSize: 11, color: "#4a4540", marginBottom: 7, fontFamily: "'DM Mono'", letterSpacing: ".07em", textTransform: "uppercase" }}>{t}</div>;
  const Tip = ({ s, children }) => <div style={{ padding: "12px 14px", background: "#fff7ed", borderRadius: 8, border: "1px solid #fed7aa", borderLeft: "3px solid #f97316", fontSize: 13, color: "#7c2d12", fontFamily: "'DM Sans'", lineHeight: 1.6, marginTop: 14 }}><strong style={{ color: "#c05500" }}>{s} </strong>{children}</div>;

  const priBadge = p => {
    const m = { kritik: { bg: "#fef2f2", c: "#dc2626", border: "#fca5a5" }, yüksek: { bg: "#fff7ed", c: "#c05500", border: "#fed7aa" }, orta: { bg: "#f5f0eb", c: "#4a4540", border: "#e8e2da" } };
    const s = m[p] || m.orta;
    return { fontSize: 9, padding: "2px 8px", borderRadius: 99, background: s.bg, color: s.c, border: `1px solid ${s.border}`, fontFamily: "'DM Mono'", fontWeight: 600, letterSpacing: ".06em", textTransform: "uppercase" };
  };
    ...
    if (sid === "techstack") return (
      <div>
        ...
        {Object.entries(TECH_OPTIONS).map(([cat, opts]) => {
          const names = { frontend: "Frontend", backend: "Backend", database: "Veritabanı", styling: "UI / Styling", deploy: "Deploy" };
          return (
  ...
  return (
    <div style={{ display: "flex", height: "100vh", fontFamily: "'DM Sans',sans-serif", background: "#fdfaf7", overflow: "hidden" }}>
      <link href="https://fonts.googleapis.com/css2?family=Syne:wght@400;500;600;700;800&family=DM+Sans:ital,wght@0,300;0,400;0,500;0,600;0,700&family=DM+Mono:wght@400;500&display=swap" rel="stylesheet" />

      {/* Sidebar */}
      <div style={{ width: 218, background: "#1a1a1a", display: "flex", flexDirection: "column", flexShrink: 0 }}>
        <div style={{ padding: "18px 16px 14px", borderBottom: "1px solid #2a2520" }}>
          <div style={{ fontFamily: "'Syne'", fontSize: 14, fontWeight: 700, letterSpacing: ".06em", color: "#fdfaf7" }}>Dipten<span style={{ color: "#f97316" }}>Zirveye</span><sup style={{ fontSize: 8, color: "#f97316" }}>™</sup></div>
          <div style={{ fontFamily: "'DM Mono'", fontSize: 9, letterSpacing: ".12em", textTransform: "uppercase", color: "#4a4540", marginTop: 2 }}>
        ...
        <div style={{ flex: 1, padding: "8px 6px", overflowY: "auto" }}>
          {STEPS.map((s, i) => {
            ...
            return (
              <button key={s.id + i} onClick={() => setStep(i)} style={{
                display: "flex", alignItems: "center", gap: 9, width: "100%",
                padding: "7px 9px", borderRadius: 6, border: "none",
                background: ac ? "rgba(249,115,22,.15)" : "transparent",
                color: ac ? "#f97316" : dn ? "#9a9590" : "#4a4540",
                cursor: "pointer", fontSize: 12.5, fontWeight: ac ? 700 : 500,
                fontFamily: "'DM Sans'", marginBottom: 1, transition: "all .15s", textAlign: "left",
                borderLeft: ac ? "3px solid #f97316" : "3px solid transparent",
              }}>
                <span style={{ fontFamily: "'DM Mono'", fontSize: 10, fontWeight: 600, minWidth: 14, color: ac ? "#f97316" : dn ? "#22c55e" : "#4a4540" }}>{dn ? "✓" : s.tag}</span>
                {s.title}
              </button>
            );
          })}
        </div>
        <div style={{ padding: "12px 14px", borderTop: "1px solid #2a2520" }}>
          <div style={{ fontFamily: "'DM Mono'", fontSize: 9, letterSpacing: ".1em", textTransform: "uppercase", color: "#4a4540", marginBottom: 5 }}>ilerleme</div>
          <div style={{ height: 4, background: "#2a2520", borderRadius: 2, overflow: "hidden" }}>
            <div style={{ height: "100%", background: "linear-gradient(90deg,#f97316,#fbbf24)", width: `${progress}%`, borderRadius: 2, transition: "width .4s" }} />
          </div>
          <div style={{ fontFamily: "'DM Mono'", fontSize: 10, color: "#f97316", textAlign: "right", marginTop: 4, fontWeight: 600 }}>{progress}%</div>
        </div>
      </div>

      {/* Main */}
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
        <div ref={ref} style={{ flex: 1, overflowY: "auto", padding: "24px 32px 80px" }}>
          <div style={{ maxWidth: 600, margin: "0 auto" }}>{renderStep()}</div>
        </div>
        {step > 0 && (
          <div style={{ padding: "11px 32px", borderTop: "1px solid #e8e2da", display: "flex", justifyContent: "space-between", alignItems: "center", background: "rgba(253,250,247,.95)", backdropFilter: "blur(8px)" }}>
            <button onClick={() => setStep(Math.max(0, step - 1))} style={{ padding: "8px 20px", borderRadius: 7, border: "1.5px solid #e8e2da", background: "transparent", fontSize: 13, fontWeight: 600, cursor: "pointer", color: "#4a4540", fontFamily: "'DM Sans'" }}>← Geri</button>
            <span style={{ fontFamily: "'DM Mono'", fontSize: 11, color: "#9a9590" }}>{step}/{STEPS.length - 1}</span>
            {step < STEPS.length - 1 ? (
              <button onClick={() => setStep(Math.min(STEPS.length - 1, step + 1))} style={{ padding: "8px 20px", borderRadius: 7, border: "none", background: "#f97316", fontSize: 13, fontWeight: 700, cursor: "pointer", color: "#fdfaf7", fontFamily: "'Syne'", boxShadow: "0 2px 12px rgba(249,115,22,.3)", letterSpacing: ".04em" }}>Sonraki →</button>
            ) : (
              <button onClick={() => { setStep(0); setA(p => ({ ...p, mode: "" })); setRTab("fixes"); }} style={{ padding: "8px 20px", borderRadius: 7, border: "none", background: "#22c55e", fontSize: 13, fontWeight: 700, cursor: "pointer", color: "#fff", fontFamily: "'Syne'" }}>Baştan Başla</button>
            )}
          </div>
        )}
      </div>
